...
var _subscriber_id_counter = 0; // UINT32
var __SUBSCRIBER_ID__      = "__SUBSCRIBER_ID__";
// --- class / interfaces ----------------------------------
...
function MessagePassing_id(subscriber) { // @arg SubscriberObject
    ...
    if (subscriber) {
        return subscriber[__SUBSCRIBER_ID__] || ""; // __SUBSCRIBER_ID__ is hidden property
    }
    return "";
}
...
//}@dev
    ...
    // Subscriber ID は複数の MessagePassing インスタンスに登録することが可能。
    // 1つのObject/Instance に対して1つしか割り当てられず上書きもされない
    var id = subscriber[__SUBSCRIBER_ID__] || "";

    if (!id) {
        id =  subscriber ? _getConstructorName(subscriber) : "anon";
        id += "." + (++_subscriber_id_counter).toString();
        if (Object["defineProperty"]) { // [ES5]
            Object["defineProperty"](subscriber, __SUBSCRIBER_ID__, { "value": id }); // hidden and shield
        } else { // legacy browser
            subscriber[__SUBSCRIBER_ID__]  = id;
        }
